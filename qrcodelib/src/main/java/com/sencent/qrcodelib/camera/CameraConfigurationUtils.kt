...
object CameraConfigurationUtils {
    ...
    private const val MAX_ASPECT_DISTORTION = 0.10
    ...
    fun findBestPreviewSizeValue(parameters: Camera.Parameters, screenResolution: Point): Point {
        ...
        if (!rawSupportedSizes.isEmpty()) {
            rawSupportedSizes.sortWith(Comparator { o1, o2 ->
                val delta1 = Math.abs(o1.height - screenResolution.x)
                val delta2 = Math.abs(o2.height - screenResolution.x)
                delta1 - delta2
            })
            val bestPreview = rawSupportedSizes[0]
            val bestSize = Point(bestPreview.width, bestPreview.height)
